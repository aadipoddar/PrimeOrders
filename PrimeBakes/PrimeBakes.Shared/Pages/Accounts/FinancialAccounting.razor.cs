...
public partial class FinancialAccounting
{
	...
	private async Task LoadExistingSale()
	{
		try
		{
			if (Id.HasValue)
			{
				...
				{
					await ShowToast("Accounting Not Found", "The requested accounting could not be found.", "error");
		...
		{
			await ShowToast("An Error Occurred While Loading Sale Data", ex.Message, "error");
			await DeleteLocalFiles();
		}
		finally
		{
	...
	private async Task LoadExistingCart()
	{
		try
		{
			...
			if (_accounting.Id > 0)
			{
				var existingCart = await AccountingData.LoadAccountingDetailByAccounting(_accounting.Id);

				foreach (var item in existingCart)
				{
					if (_ledgers.FirstOrDefault(s => s.Id == item.LedgerId) is null)
					{
						await ShowToast("Ledger Not Found", $"The ledger with ID {item.LedgerId} in the existing accounting cart was not found in the available ledgers list. It may have been deleted or is inaccessible.", "error");
						continue;
					}
	...
	private async Task GetReferences()
	{
		...
		try
		{
			...
			var allLedgerTransactions = await AccountingData.LoadAccountingLedgerOverviewByDate(
				_selectedFinancialYear.StartDate.ToDateTime(new TimeOnly()),
	...
	private async Task UpdateFinancialDetails()
	{
		...
		_totalCredit = _cart.Sum(x => x.Credit ?? 0);
		_totalDebit = _cart.Sum(x => x.Debit ?? 0);
		_difference = _totalDebit - _totalCredit;
	...
	private async Task SaveAccountingFile()
	{
		...
		{
			await ShowToast("An Error Occurred While Saving Accounting Data", ex.Message, "error");
		}
		finally
		{
	...
	private async Task<bool> ValidateForm()
	{
		...
		if (_totalCredit != _totalDebit || _difference != 0)
		{
			await ShowToast("Credit and Debit Mismatch", "The total credit and debit amounts in the cart do not match. Please ensure they are equal before saving.", "error");
			return false;
		}
