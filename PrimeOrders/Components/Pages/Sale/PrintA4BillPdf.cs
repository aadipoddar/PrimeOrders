...
internal static class PrintA4BillPdf
{
	private static readonly Color PrimaryColor = Color.FromArgb(81, 43, 212);
	private static readonly Color SecondaryColor = Color.FromArgb(104, 33, 122);
	private static readonly Color AccentColor = Color.FromArgb(0, 164, 239);
	private static readonly Color LightGray = Color.FromArgb(245, 245, 245);
	private static readonly Color DarkGray = Color.FromArgb(64, 64, 64);

	// Page layout constants
	private const float PageMargin = 20f;
	private const float RowHeight = 25f;
	private const float HeaderHeight = 30f;
	private const float FooterReservedSpace = 100f; // Space reserved for footer content

	internal static async Task<byte[]> GenerateA4SaleBill(SaleModel sale, List<SaleProductCartModel> saleProducts)
	{
		try
		{
		...
		{
			// Log error and return empty array or rethrow based on requirements
		...
		float startY, float pageWidth, float pageHeight, PdfFont subHeaderFont, PdfFont normalFont, PdfFont boldFont)
	{
		try
		{
			...
			var maxYForContent = pageHeight - FooterReservedSpace;

			// Draw table rows with pagination
			for (int i = 0; i < saleProducts.Count; i++)
			{
				// Check if we need a new page
				if (currentY + RowHeight > maxYForContent)
				{
					// Add a new page
		...
		float col1X, float col2X, float col3X, float col4X, float col5X, float col6X, PdfFont boldFont)
	{
		try
		{
			...
			var tableHeaderRect = new RectangleF(tableStartX, currentY, tableWidth, HeaderHeight);
			graphics.DrawRectangle(new PdfPen(Color.Black, 1), new PdfSolidBrush(PrimaryColor), tableHeaderRect);

			// Draw column separators for header
			DrawColumnSeparators(graphics, currentY, HeaderHeight, col1X, col2X, col3X, col4X, col5X, col6X, tableStartX + tableWidth);
		...
		PdfFont normalFont, PdfFont boldFont)
	{
		...
		{
			// Draw row background
			var rowRect = new RectangleF(tableStartX, currentY, tableWidth, RowHeight);
			...
			DrawColumnSeparators(graphics, currentY, RowHeight, col1X, col2X, col3X, col4X, col5X, col6X, tableStartX + tableWidth);
			...
			DrawRightAlignedText(graphics, $"₹{product.Total:N2}", boldFont, new PdfSolidBrush(SecondaryColor), col6X, col6Width, rowY);
		}
		catch (Exception)
		{
			// Ignore row drawing errors
		}
	}
		...
		try
		{
			for (int i = 0; i < document.Pages.Count; i++)
			{
				...
				if (i == document.Pages.Count - 1)
				{
					...
					graphics.DrawString(thankYouText, normalFont, new PdfSolidBrush(PrimaryColor),
						new PointF((pageWidth - thankYouSize.Width) / 2, footerY));
				}

				// Footer line
				graphics.DrawLine(new PdfPen(LightGray, 1), new PointF(20, footerY + 20), new PointF(pageWidth - 20, footerY + 20));
				...
				graphics.DrawString(systemInfo, smallFont, new PdfSolidBrush(DarkGray),
	...
	private static async Task<float> DrawHeader(PdfGraphics graphics, float pageWidth, float currentY,
		PdfFont titleFont, PdfFont headerFont, PdfFont normalFont, int locationId)
	{
		try
		{
			...
			graphics.DrawRectangle(new PdfSolidBrush(PrimaryColor), headerRect);
			...
			graphics.DrawRectangle(new PdfPen(LightGray, 1), new PdfSolidBrush(LightGray), detailsRect);

			var detailsY = currentY + 10;
			graphics.DrawString("GST NO: XXXXXXX", normalFont, new PdfSolidBrush(DarkGray), new PointF(30, detailsY));
			graphics.DrawString("Salasar Foods Guwahati", normalFont, new PdfSolidBrush(DarkGray), new PointF(30, detailsY + 15));
			graphics.DrawString("Mobile No: XXXXXX", normalFont, new PdfSolidBrush(DarkGray), new PointF(30, detailsY + 30));
			graphics.DrawString("Email: info@primeorders.com", normalFont, new PdfSolidBrush(DarkGray), new PointF(30, detailsY + 45));

			// Invoice title on the right
			var invoiceTitle = "SALES INVOICE";
			var invoiceTitleSize = headerFont.MeasureString(invoiceTitle);
			graphics.DrawString(invoiceTitle, headerFont, new PdfSolidBrush(SecondaryColor),
		...
		{
			// Return default position if header drawing fails
			return currentY + 170;
		}
	}

	private static float DrawInvoiceDetails(PdfGraphics graphics, float pageWidth, float currentY,
		PdfFont subHeaderFont, PdfFont normalFont, PdfFont boldFont, SaleModel sale)
	{
		try
		{
			...
			graphics.DrawString("Invoice Details", subHeaderFont, new PdfSolidBrush(PrimaryColor), new PointF(leftX, detailY));

			detailY += 25;
			graphics.DrawString("Invoice No: ", normalFont, PdfBrushes.Black, new PointF(leftX, detailY));
			graphics.DrawString($"{sale.BillNo ?? "N/A"}", boldFont, new PdfSolidBrush(SecondaryColor), new PointF(leftX + 70, detailY));
		...
		PdfFont subHeaderFont, PdfFont normalFont, SaleModel sale)
	{
		try
		{
			...
			if (party == null) return currentY;

			// Customer info section
			var customerRect = new RectangleF(20, currentY, pageWidth - 40, 60);
			graphics.DrawRectangle(new PdfPen(Color.Black, 1), new PdfSolidBrush(LightGray), customerRect);

			var customerY = currentY + 10;
			graphics.DrawString("Bill To:", subHeaderFont, new PdfSolidBrush(PrimaryColor), new PointF(30, customerY));
	...
	private static void DrawCenteredText(PdfGraphics graphics, string text, PdfFont font, PdfBrush brush, float columnX, float columnWidth, float y)
	{
		...
		{
			// Fallback to left-aligned if centering fails
			graphics.DrawString(text, font, brush, new PointF(columnX + 5, y));
		}
	}
	...
	private static void DrawRightAlignedText(PdfGraphics graphics, string text, PdfFont font, PdfBrush brush, float columnX, float columnWidth, float y)
	{
		...
		{
			// Fallback to left-aligned if right alignment fails
			graphics.DrawString(text, font, brush, new PointF(columnX + 5, y));
		}
	}

	private static float DrawSummarySection(PdfGraphics graphics, float pageWidth, float currentY,
		PdfFont subHeaderFont, PdfFont normalFont, PdfFont boldFont, List<SaleProductCartModel> saleProducts, SaleModel sale)
	{
		try
		{
			if (saleProducts == null || saleProducts.Count == 0)
			...
			graphics.DrawRectangle(new PdfPen(Color.Black, 1), new PdfSolidBrush(LightGray), summaryRect);

			var summaryY = currentY + 10;
			graphics.DrawString("Summary", subHeaderFont, new PdfSolidBrush(PrimaryColor), new PointF(summaryX + 10, summaryY));
			...
			graphics.DrawRectangle(new PdfPen(PrimaryColor, 2), new PdfSolidBrush(PrimaryColor), grandTotalRect);
	...
	private static void DrawSummaryLine(PdfGraphics graphics, string label, string value, float summaryX, float summaryWidth, float y,
		PdfFont normalFont, PdfFont boldFont)
	{
		try
		{
			...
			graphics.DrawString(value, boldFont, new PdfSolidBrush(SecondaryColor),
				new PointF(summaryX + summaryWidth - valueSize.Width - 15, y));
		}
		catch (Exception)
		{
			// Ignore drawing errors for individual lines
		}
	}

	private static float DrawAmountInWords(PdfGraphics graphics, float pageWidth, float currentY,
		PdfFont normalFont, PdfFont boldFont, decimal total)
	{
		try
		{
			...
			graphics.DrawString("Amount in Words:", boldFont, new PdfSolidBrush(PrimaryColor), new PointF(30, currentY + 8));
	...
	private static float DrawPaymentInfo(PdfGraphics graphics, float pageWidth, float currentY,
		PdfFont normalFont, PdfFont boldFont, SaleModel sale)
	{
		try
		{
			...
			graphics.DrawString(paymentMode, boldFont, new PdfSolidBrush(SecondaryColor), new PointF(120, currentY + 8));
