...
public static class SaleA4Print
{
	...
	public static async Task<MemoryStream> GenerateA4SaleBill(int saleId)
	{
		...
		pdfDocument.Template.Top = DrawHeader(pdfDocument, location.Name);
		pdfDocument.Template.Bottom = DrawFooter(pdfDocument);

		float currentY = DrawCompanyInformation(pdfPage, "SALES INVOICE");
		currentY = DrawInvoiceDetails(pdfPage, currentY, sale);
		var result = DrawItemDetails(pdfPage, currentY, saleProductCartModel);
		currentY = DrawSummary(pdfDocument, result.Page, result.Bounds.Bottom + 20, sale);
	...
	private static PdfPageTemplateElement DrawHeader(PdfDocument doc, string locationName)
	{
